...
<div align=center>
<img width="600" src="../img/book-org.svg"/>
</div>
<center>图1 全书结构</center>


* 第一部分包括基础知识和预备知识。第1章提供深度学习的入门课程。然后在第2章中，我们将快速向你介绍实践深度学习所需的前提条件，例如如何存储和处理数据，以及如何应用基于线性代数、微积分和概率基本概念的各种数值运算。 第3和第4章涵盖了深度学习的最基本概念和技术，例如线性回归、多层感知机和正则化。

* 接下来的五章集中讨论现代深度学习技术。第5章描述了深度学习计算的各种关键组件，并为我们随后实现更复杂的模型奠定了基础。接下来，在第6和第7章中，我们介绍了卷积神经网络（convolutional neural network，CNN），这是构成大多数现代计算机视觉系统骨干的强大工具。随后，在第8和第9章中，我们引入了循环神经网络(recurrent neural network，RNN)，这是一种利用数据中的时间或序列结构的模型，通常用于自然语言处理和时间序列预测。在第10章中，我们介绍了一类新的模型，它采用了一种称为注意力机制的技术，最近它们已经开始在自然语言处理中取代循环神经网络。这一部分将帮助你快速了解大多数现代深度学习应用背后的基本工具。

* 第三部分讨论可伸缩性、效率和应用程序。
首先，在第11章中，我们讨论了用于训练深度学习模型的几种常用优化算法。第12章将探讨影响深度学习代码计算性能的几个关键因素。在第13章中，我们展示了深度学习在计算机视觉中的主要应用。在第14和第15章中，我们展示了如何预训练语言表示模型并将其应用于自然语言处理任务。
...
有时，为了避免不必要的重复，我们将本书中经常导入和引用的函数、类等封装在utils模块中。

本书中的大部分代码都是基于[OneFlow](https://github.com/Oneflow-Inc/oneflow)的。


### 目标受众

本书面向学生（本科生或研究生）、工程师和研究人员，他们希望扎实掌握深度学习的实用技术。因为我们从头开始解释每个概念，所以不需要过往的深度学习或机器学习背景。全面解释深度学习的方法需要一些数学和编程，但我们只假设你了解一些基础知识，包括线性代数、微积分、概率和非常基础的Python编程。此外，在附录中，我们提供了本书所涵盖的大多数数学知识的复习。大多数时候，我们会优先考虑直觉和想法，而不是数学的严谨性。有许多很棒的书可以引导感兴趣的读者走得更远。Bela Bollobas的《线性分析》[2]对线性代数和函数分析进行了深入的研究。 Wasserman的《All of statistics: a concise course in statistical inference》[3]是一本很好的统计学指南。如果你以前没有使用过Python语言，那么你可能想要仔细阅读这个[Python教程](http://learnpython.org/)。
...
## 参考文献
[1] PBishop, C. M. (2006). Pattern recognition and machine learning. springer.

[2] Bollobás, B. (1999). Linear analysis. Cambridge University Press, Cambridge.

[3] Wasserman, L. (2013). All of statistics: a concise course in statistical inference. Springer Science & Business Media.