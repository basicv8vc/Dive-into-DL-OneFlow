...
回想一下，在多项式回归的例子（4.4节）中，我们可以通过调整拟合多项式的阶数来限制模型的容量。实际上，限制特征的数量是缓解过拟合的一种常用技术。然而，简单地丢弃特征对于这项工作来说可能过于生硬。我们继续思考多项式回归的例子，考虑高维输入可能发生的情况。多项式对多变量数据的自然扩展称为*单项式*（monomials），也可以说是变量幂的乘积。单项式的阶数是幂的和。例如，$x_1^2 x_2$和$x_3 x_5^2$都是3次单项式。
...
现在，如果我们的权重向量增长的太大，我们的学习算法可能会更集中于最小化权重范数$\| \mathbf{w} \|^2$。这正是我们想要的。让我们回顾一下3.1节中的线性回归例子。我们的损失由下式给出：
...
true_w, true_b = flow.ones(num_inputs, 1) * 0.01, 0.05
...
```python
def init_params():
    w = flow.randn(num_inputs, 1, requires_grad=True)
...
```python
def l2_penalty(w):
    return flow.sum(w.pow(2)) / 2
```

### 4.5.3.3. 定义训练代码实现

下面的代码将模型拟合训练数据集，并在测试数据集上进行评估。从第3章以来，线性网络和平方损失没有变化，所以我们通过`utils.linreg`和`utils.squared_loss`导入它们。唯一的变化是损失现在包括了惩罚项。
...
def train(lambd):
    ...
    print('w的L2范数是：', flow.linalg.norm(w).item())
```
...
train(lambd=0)
```
    w的L2范数是： 13.355674743652344

<div align=center>
<img src="../img/output_weight-decay_ec9cc0_66_1.svg"/>
</div>
...
train(lambd=3)
```
    w的L2范数是： 0.36988565325737

<div align=center>
<img src="../img/output_weight-decay_ec9cc0_78_1.svg"/>
</div>
...
def train_concise(wd):
    ...
    loss = nn.MSELoss()
    num_epochs, lr = 100, 0.003
    print(type(net[0].weight), type(net[0].bias))
    # 偏置参数没有衰减。
    trainer = flow.optim.SGD([
...
train_concise(0)
```
    w的L2范数： 13.808016777038574

<div align=center>
<img src="../img/output_weight-decay_ec9cc0_106_1.svg"/>
</div>

```python
train_concise(3)
```
    w的L2范数： 0.4968254268169403

<div align=center>
<img src="../img/output_weight-decay_ec9cc0_107_1.svg"/>
</div>
...
1. 我们知道$\|\mathbf{w}\|^2 = \mathbf{w}^\top \mathbf{w}$。你能找到类似的矩阵方程吗（见2.3.10节中的弗罗贝尼乌斯范数）？
