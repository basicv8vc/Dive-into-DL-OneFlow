...
3. 通过标准的特征提取算法（如SIFT（尺度不变特征变换）[1] 、SURF（加速鲁棒特征）[2]或其他手动调整的流水线来输入数据。
...
另一种预测这个领域发展的方法————观察图像特征的提取方法。在2012年前，图像特征都是机械地计算出来的。事实上，设计一套新的特征函数、改进结果，并撰写论文是盛极一时的潮流。SIFT[2]、SURF[2]、HOG（定向梯度直方图）[3]、[bags of visual words](https://en.wikipedia.org/wiki/Bag-of-words_model_in_computer_vision) 和类似的特征提取方法占据了主导地位。
...
有趣的是，在网络的最底层，模型学习到了一些类似于传统滤波器的特征抽取器。图7.1.1是从AlexNet论文[4]复制的，描述了底层图像特征。

<div align=center>
<img width=400 src="../img/filters.png"/>
</div>
<center>图7.1.1 AlexNet第一层学习到的特征抽取器。</center>
...
<div align=center>
<img width=400 src="../img/alexnet.svg"/>
</div>
<center>图7.1.2 从LeNet（左）到AlexNet（right）</center>
...
AlexNet通过dropout（4.6节）控制全连接层的模型复杂度，而LeNet只使用了权重衰减。
为了进一步扩充数据，AlexNet在训练时增加了大量的图像增强数据，如翻转、裁切和变色。
这使得模型更健壮，更大的样本量有效地减少了过拟合。
我们将在13.1节中更详细地讨论数据扩充。
...
```
    Conv2d Output shape:	 flow.Size([1, 96, 54, 54])
    ReLU Output shape:	 flow.Size([1, 96, 54, 54])
    MaxPool2d Output shape:	 flow.Size([1, 96, 26, 26])
    Conv2d Output shape:	 flow.Size([1, 256, 26, 26])
    ReLU Output shape:	 flow.Size([1, 256, 26, 26])
    MaxPool2d Output shape:	 flow.Size([1, 256, 12, 12])
    Conv2d Output shape:	 flow.Size([1, 384, 12, 12])
    ReLU Output shape:	 flow.Size([1, 384, 12, 12])
    Conv2d Output shape:	 flow.Size([1, 384, 12, 12])
    ReLU Output shape:	 flow.Size([1, 384, 12, 12])
    Conv2d Output shape:	 flow.Size([1, 256, 12, 12])
    ReLU Output shape:	 flow.Size([1, 256, 12, 12])
    MaxPool2d Output shape:	 flow.Size([1, 256, 5, 5])
    Flatten Output shape:	 flow.Size([1, 6400])
...
现在，我们可以开始训练AlexNet了。与6.6节中的LeNet相比，这里的主要变化是使用更小的学习速率训练，这是因为网络更深更广、图像分辨率更高，训练卷积神经网络就更昂贵。

```python
lr, num_epochs = 0.1, 10
train_ch6(net, train_iter, test_iter, num_epochs, lr, try_gpu())
```
    loss 0.273, train acc 0.899, test acc 0.873
1089.2 examples/sec on cuda:0
...
[2] Bay, H., Tuytelaars, T., & Van Gool, L. (2006). Surf: speeded up robust features. European conference on computer vision (pp. 404–417).

[3] Dalal, N., & Triggs, B. (2005). Histograms of oriented gradients for human detection. 2005 IEEE computer society conference on computer vision and pattern recognition (CVPR’05) (pp. 886–893).
